...
function renderUserData(data) {
    const { userName, userImg, userEmail, facebookLogin } = data;
    document.getElementById("contenedorNombreDeUsuario").innerHTML = userName;

    if(facebookLogin) {
        const html = `
            <img id="contenedorImgDeUsuario" class="rounded-circle" src="${userImg}" alt="foto de perfil del usuario">
            <div id="contenedorEmailDeUsuario">${userEmail}</div>`;
        
        document.getElementById("contenedorFacebookLogin").innerHTML = html; 
    }
}
